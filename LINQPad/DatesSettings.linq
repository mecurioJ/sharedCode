...
void Main()
{
		...
		var knownDateTypes = new[]{"Years","Quarters","Months","DayOfWeek","Date"};
		
		
		OlapData.Dimensions.SelectMany(cDim => cDim.Attributes)
		.Where(cAtt => knownDateTypes.Contains(cAtt.Type))
		.Distinct().Dump();

	var sourceSet = new[]{
		"[Calendar Period].[Calendar Period].[Date].&[142]",
		"[Calendar Period].[Day of Week].&[3]",
		"[Calendar Period].[Weekly Calendar Period].[Year].&[2011]",
		"[Calendar Period].[Month Name].&[4]",
...
var selectionSet = 
sourceSet.Select(itm => itm.Replace("]", String.Empty).Replace("[", String.Empty).Split('.'))
                .Select(mbr =>
                {
                    var dimension = mbr[0];
                    var hierarchy = mbr[1];
                    var level = (mbr.Count() > 2 && !mbr[2].Contains("&")) ? mbr[2] : String.Empty;
                    var value = (mbr.Any(x => x.Contains("&"))) ? mbr.FirstOrDefault(xx => xx.Contains("&")) : string.Empty;
                ...
                }).DistinctBy(d => String.Format("{0}.{1}.{2}", d.dimension, d.hierarchy, d.level))
				.OrderBy(xx => xx.dimension);

	var DateDimensions = GetDateDimensions(MetaDataConnection,CatalogName,"Loans")
			.Select(tt => new{
				DimensionUniqueName = tt.Item1,
				DimensionCaption = tt.Item2,
				DimensionName = tt.Item3,
				DefaultHierarchy = tt.Item4
			});
	
	  //var Restrictions = new Microsoft.AnalysisServices.AdomdClient.AdomdRestrictionCollection();
	  var getDateSelections = selectionSet.Join(DateDimensions,ss => ss.dimension, dd => dd.DimensionName, (ss,dd) => ss);
	  
	  var DateMembers = getDateSelections.Dump();
	  var NonDateMembers = selectionSet.Except(getDateSelections).Dump();
}

public IEnumerable<System.Tuple<String,String,String,String>> GetDateDimensions(AdomdConnection MetaDataConnection, String catalogName, String cubeName)
{
	
            IEnumerable<System.Tuple<String,String,String,String>> quadruple;
	if (MetaDataConnection.State != ConnectionState.Open)
	{
		MetaDataConnection.Open();	  	
	}
	
	quadruple =  MetaDataConnection
					.GetSchemaDataSet(
						AdomdSchema.sSchemaList[AdomdSchemaGuid.Dimensions].Id,new Microsoft.AnalysisServices.AdomdClient.AdomdRestrictionCollection()
							{
								...
								//new Microsoft.AnalysisServices.AdomdClient.AdomdRestriction("PROPERTY_CONTENT_TYPE",177),
							    //new Microsoft.AnalysisServices.AdomdClient.AdomdRestriction("MEMBER_UNIQUE_NAME","[Calendar Period].[Calendar Period].[Date].&[142]"),
								
							})
							.Tables[0]
							.Rows.Cast<DataRow>()
							//Get all Dimensions where type is Time
							.Where(dr => dr.Field<Int16>("DIMENSION_TYPE").Equals(1))
							//Get all Levels where type is time							
							.Select(dr => 
								System.Tuple.Create(
								dr.Field<String>("DIMENSION_UNIQUE_NAME"),
								dr.Field<String>("DIMENSION_CAPTION"),
								dr.Field<String>("DIMENSION_NAME"),
								dr.Field<String>("DEFAULT_HIERARCHY")
								));
