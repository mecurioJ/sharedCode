...
void Main()
{
	 AdomdConnection MetaDataConnection = new AdomdConnection(Utils.ConnectionString(ServerName, CatalogName));
      if (MetaDataConnection.State != ConnectionState.Open)
      {
		...
		//make a big list of denormalized columns and tables...
		
		svr.Databases.Cast<Microsoft.AnalysisServices.Database>().ToArray().Select(db => db.DataSourceViews.Cast<Microsoft.AnalysisServices.DataSourceView>())
		.First()
		.Select(t1 => new{
			t1.Name,
			t1.ID,
			SchemaTables = t1.Schema.Tables.Cast<DataTable>().SelectMany(tbl => 
					tbl.Columns.Cast<DataColumn>().Union(tbl.PrimaryKey.Cast<DataColumn>()).Select(col => new{
					IsPrimaryKey = tbl.PrimaryKey.Cast<DataColumn>().Contains(col),
					col.Table.TableName,
					col.ColumnName,
					DataSourceViewName = t1.Name,
					DataSourceViewId = t1.ID,
					col.AllowDBNull,
					col.AutoIncrement,
					...
					col.DefaultValue,
					//col.Expression,
					Expression = (col.ExtendedProperties["ComputedColumnExpression"] != null) ? col.ExtendedProperties["ComputedColumnExpression"].ToString() : String.Empty,
		...
		}).SelectMany(t1 => t1.SchemaTables.Where(tCol => !String.IsNullOrEmpty(tCol.Expression))
			.Select(st => new{st.TableName,st.ColumnName,st.DataSourceViewId,st.DataSourceViewName, 
			Expression = st.Expression.Replace("\n", "||").Replace("\r","||").Replace("||||", " ")
			}).Select(ex => String.Format("\"{0}\";",ex.Expression))
...
//			cCol.ColumnName,
//			cCol.DataType,
//			DbTableName = cCol.Table.ExtendedProperties["DbTableName"].ToString(),
//			TableFriendlyName = cCol.Table.ExtendedProperties["FriendlyName"].ToString(),
...
//									.Replace("\r", " ")
//							: String.Empty,
//			DbColumnName = cCol.ExtendedProperties["DbColumnName"].ToString(),
...
//							.Where(ds => ds.TableName.Equals(((ColumnBinding)di.Source).TableID) && ds.ColumnName.Equals(((ColumnBinding)di.Source).ColumnID)));
//							
