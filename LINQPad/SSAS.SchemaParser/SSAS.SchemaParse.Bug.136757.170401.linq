...
void Main()
{
	SchemaParser.oDataBase OlapData = new SchemaParser.oDataBase(XElement.Load(@"c:\Projects\SSAS\BankerSchema.xml"));
			...
			//dataTableExpressions.SelectMany(dt => dt.ExpressionTree.Where(exp => !String.IsNullOrEmpty(exp.Expression))).Dump();
			
			var cubeName = "Global";
			var MeasureGroup = "Global Facts";
			
			MeasureGroupDimensions.Dump();
