...
void Main()
{
	SchemaParser.oDataBase OlapData = new SchemaParser.oDataBase(XElement.Load(@"c:\Projects\SSAS\BankerSchema.xml"));
            ...
            String cubeName = "GL Cube";
            String measureGroupName = "Global Facts";
            List<String> filterList = new List<String>(){
				"[Date].[Calendar Period].[All Calendar Periods]",
				"[Chart of Accounts].[Report Type].&[B]",
				};
            List<String> columnList = new List<String>(){"[Measures].[Current Balance]"}; 
            List<String> rowList = new List<String>(){"[Chart of Accounts].[Chart of Accounts].&[10299]"};
			
			OlapData.Cubes.Where(cub => cub.Name.Equals(cubeName)).Dump();
			
			SchemaParser.GetMeasureGroupMeasureDefinitions(OlapData)
			.Where(mg => mg.CubeName.Equals(cubeName))
			.Dump();
	        ...
	        .Select(spx => new{
		        Member = spx//[0]
					        ...
					        .Replace("]",String.Empty)
					        .Split('.'),
		        Values = (Regex.Split(spx,".&").Count() > 1) ? Regex.Split(spx,".&")[1] : String.Empty
	        }).Select(mem => new{
		        Dimension = mem.Member[0],
		        Hierarchy = mem.Member[1],
		        Level = (mem.Member.Count() > 2) && !mem.Member[2].Contains("&")
				        ? mem.Member[2]
				        : String.Empty,
		        Value = mem.Values
					        .Replace("[",String.Empty)
            ...
            SchemaParser.GetMeasureGroupMeasureDefinitions(OlapData)
                        ...
                        .Where(mgNm => mgNm.MeasureGroupName.Equals(measureGroupName))
                        .Where(ms => filterList.Union(columnList.Union(rowList))
                        .Where(item => item.Contains("[Measures]"))
                        .Select(item => item
                            .Replace("[", String.Empty)
                            .Replace("]", String.Empty)
                            .Split('.')[1]).Contains(ms.MeasureName))
                        .Select(mDef => new
                            {
                                TableName = mDef.SourceDataTableName,
                                ColumnName = mDef.SourceDataColumnName
                            });
...
// oHier.AllMemberName
	        ...
	        .SelectMany(cub => 
		        ...
		        .SelectMany(dSrc => 
			        ...
			        )).Union(OlapData.Cubes.Where(cub => cub.Name.Equals(cubeName))
	        .SelectMany(cub => 
		        cub.Dimensions//.Where(cDim => memberSet.Select(ms => ms.Dimension).Contains(cDim.Name))
		        .SelectMany(dSrc => 
			        OlapData.Dimensions.Where(oDim => oDim.ID.Equals(dSrc.DimensionID))
		...
		var ExpressionColumns = relationTables.SelectMany(tbl =>
            ...
            var JoinSet = relationTables.SelectMany(tbl => tbl.DataSourceViewRelations.Select(rel =>
                            ...
                            })).Where(lt => TableSet.Select(k => k.Key).Contains(lt.LeftTable) && TableSet.Select(k => k.Key).Contains(lt.RightTable)).ToArray();
		
		var RelationalSet = 
                     new //RelationalBuilderContext()
                        {
                            DatabaseId = databaseId,
                            Tables = TableSet.Select(tbl => new 
                                {
                                    Name = tbl.Key,
                                    Columns = tbl
                                }
