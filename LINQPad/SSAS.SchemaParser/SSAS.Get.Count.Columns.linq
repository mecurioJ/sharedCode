...
var filteredCube = OlapData.Cubes.Where(cNm => cNm.Name.Equals(cubeName));



OlapData.Cubes.Where(cNm => cNm.Name.Equals(cubeName)).SelectMany(mgrp => mgrp.MeasureGroups
.SelectMany(mgp => 
	mgp.Measures.Select(msr => 
		new{
			Name = String.Format("[Measures].[{0}]",msr.Name),
			CalculationType = msr.AggregateFunction
		}).Union(mgrp.MdxScripts.SelectMany(mdxSc => mdxSc.CalculationProperties.Select(calcProp => new{
		Name = calcProp.CalculationReference.Replace("[MEASURES]","[Measures]"),
		CalculationType = "Calculation",//calcProp.CalculationType,
		//calcProp.AssociatedMeasureGroupID
	}))))).Distinct().Where(calc => calc.CalculationType.Contains("Count")).Dump();
