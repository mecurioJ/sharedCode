...
void Main()
{
	SchemaParser.oDataBase OlapData = new SchemaParser.oDataBase(XElement.Load(@"c:\Projects\SSAS\BankerSchema.xml"));
            ...
            String measureGroupName = "Products";
            IEnumerable<String> filterList = new[]{"[Calendar Period].[Calendar Period].[Date].&[142]"};
            IEnumerable<String> columnList = new[]{"[Measures].[Customer Count]"};
            IEnumerable<String> rowList = new[]{"[Responsibility].[Opened by Officer].&[AJM: Ezra Patel]"};
			
            //get Data Source View Definitions
            var dataSourceViewDef = SchemaParser.GetDataSourceViewDefinitions(OlapData);

            //get definition of Hierarchies and Levels
            var LevelDefinition = SchemaParser.GetHierarchyByLevelDefinition(OlapData);

            //Get definition of measures by measure group
            var MeasureDefinitions = SchemaParser.GetMeasureGroupMeasureDefinitions(OlapData);

            //Get definition of measure group dimensions
            var MeasureGroupDimensions = SchemaParser.GetMeasureGroupDimensionDefinitions(OlapData);
			
			//Get Attribute Definitions
			var AttributeDefs = SchemaParser.GetAttributeDefinitions(OlapData);
			...
			var DataSourceViewId = OlapData.Cubes.Where(cub => cub.Name.Equals(cubeName)).FirstOrDefault().Source;
			
			var RelationsSet = 
			SchemaParser.GetDataSourceViewDefinitions(OlapData)
			.SelectMany(dsv => dsv.Relations.Select(drel => new{
					dsv.DataSourceViewId, 
					Parent = drel.Parent.TableName, 
					Child = drel.Child.TableName
					}));
...
//how to handle when a measure is based on a dimension table							
							
//get Table and Column for Measure:
var measureSetDetails = 
measureSet
.Select(mSet => new{
	mSet.TableName,
	mSet.ColumnName,
	//PrimaryKeys = cubeSourceTablePkLookup[mSet.TableName].ToList()
}).Dump();

//get primary key for measure table when not Fact_xxxx

DataSourceViewId.Dump();
var RelationsForCube = RelationsSet.Where(rSet => rSet.DataSourceViewId.Equals(DataSourceViewId));	

//what we have to do is find the relationships between 
//	the child tables and other tables in the set.
//Then we will want to find 

var relationTableSources = relationTables.SelectMany(relT => relT.DataSourceViewRelations)
.Select(ar => new{DataSourceViewId, Parent = ar.Parent.TableName, Child = ar.Child.TableName})
.Union(RelationsForCube.Where(rSet => measureSetDetails.Select(msd => msd.TableName).Contains(rSet.Parent)))
.Select(tset => new{
	tset.Parent,
	tset.Child,
	ChildToForeignChild = RelationsForCube.Where(rSet => rSet.Child.Equals(tset.Child)),
	ParentToForeignParent = RelationsForCube.Where(rSet => rSet.Child.Equals(tset.Parent)),
	//ParentToForeignChild = RelationsForCube.Where(rSet => rSet.Parent.Equals(tset.Parent)),
	ChildToForeignParent = RelationsForCube.Where(rSet => rSet.Parent.Equals(tset.Child) && !rSet.Parent.Equals(tset.Parent))
})
//.ToLookup(g => g.Parent, g => RelationsForCube.Where(rSet => rSet.Child.Equals(g.Child)).OrderBy(oby => oby.Parent))
.Dump();
...
//dimensionHierarchyLevel.Select(dhl => new{dhl.Dimension, dhl.Hierarchy, dhl.Level, dhl.ID }).Dump();
//KeyColumns.Dump();
//ExpressionColumns.Dump();
//TableSet.Select(k => k.Key).Dump();



//Get a Set of Table Relations and find where all tables exist with relationships...
var alternateRelations = dataSourceViewDef.SelectMany(dsv => dsv.Relations.Where(dRel => 
	TableSet.Select(k => k.Key).Contains(dRel.Parent.TableName)
	|| TableSet.Select(k => k.Key).Contains(dRel.Child.TableName)
	)).Select(ar => new{parent = ar.Parent.TableName, child = ar.Child.TableName})
	.Distinct()
	.ToLookup(g => g.child, g => g.parent)
	//.Where(l => l.Intersect(TableSet.Select(k => k.Key)).Count() == TableSet.Select(k => k.Key).Count())
	;

//alternateRelations.Dump();
//JoinSet.Any().Dump();
}
...
//        }
        ...
        public static IEnumerable<MeasureDefinition> GetMeasureDefinitions(oDataBase olapData)
        {
            var SourceExpressions = GetDataSourceViewExpressions(olapData)
        .SelectMany(dsv => dsv.ExpressionTree.Select(exTree => new
        {
            dsv.DataSourceViewId,
            dsv.DataSourceViewName,
            exTree.TableName,
            exTree.ColumnName,
            exTree.Expression
        }).Where(exTree => !String.IsNullOrEmpty(exTree.Expression)));

            return olapData.Cubes.SelectMany(cub =>
                                cub.MeasureGroups.SelectMany(mg => mg.Measures.SelectMany(mea => mea.Source.SelectMany(mSrc => mSrc.Source.Select(src => new MeasureDefinition
                                {
                                    CubeName = cub.Name,
                                    CubeId = cub.ID,
                                    MeasureGroupName = mg.Name,
                                    MeasureGroupID = mg.ID,
                                    IgnoreUnrelatedDimensions = mg.IgnoreUnrelatedDimensions,
                                    MeasureName = mea.Name,
                                    MeasureID = mea.ID,
                                    MeasureDescription = mea.Description,
                                    AggregateFunction = mea.AggregateFunction,
                                    DataType = mea.DataType,
                                    Visible = mea.Visible,
                                    MeasureExpression = mea.MeasureExpression,
                                    DisplayFolder = mea.DisplayFolder,
                                    FormatString = mea.FormatString,
                                    TableName = src.TableName,
                                    ColumnName = src.ColumnName,
                                    SourceDataType = mSrc.DataType,
                                    ColumnExpression = SourceExpressions
                                        .Where(sExp => sExp.TableName.Equals(src.TableName))
                                        .Where(sExp => sExp.ColumnName.Equals(src.ColumnName))
                                        .Select(sExp => sExp.Expression).FirstOrDefault() ?? String.Empty
                                })))));
        }
