...
void Main()
{
	...
	SchemaParser.oDataBase OlapData = new SchemaParser.oDataBase(XElement.Load(@"c:\Projects\SSAS\BankerSchema.xml"));
		...
		var SourceExpressions = SchemaParser.GetDataSourceViewExpressions(OlapData)
	...
	//Isolate our Cube
	var CubeSet = OlapData.Cubes
	//.Where(cNm => cNm.Name.Equals(CubeName))
	.SelectMany(tt => tt.Dimensions
	.OrderBy(cdNm => cdNm.Name)
	.Join(
		OlapData.Dimensions,
		cDim => cDim.DimensionID,
		oDim => oDim.ID,		
		(cDim, oDim) => 		
		new{	
			AttributeHierarchies = oDim.Hierarchies.SelectMany(oHier => oHier.Levels.SelectMany(oLvl => 
				oDim.Attributes.Where(oAtt => oAtt.ID.Equals(oLvl.SourceAttributeID)).Select(oAtt => new{
					CubeName = tt.Name,
					HierarchyType = "AttributeHierarchy",
					Dimension = cDim.Name,
					Hierarchy = oHier.Name,
					Level = oLvl.Name,
					DataSourceViewID = oDim.Source,
					SourceDimensionName = oDim.Name,
					CubeDimensionName = cDim.Name,
					oHier.ID,
					oHier.AllMemberName,
					oHier.MemberNamesUnique,
					oHier.AllowDuplicateNames,
					oHier.MemberKeysUnique,
					SourceAttributeName = oAtt.Name,
					SourceLevelID = oLvl.ID,
					oLvl.SourceAttributeID,
					SourceAttribute = oAtt.ID,
					oAtt.Type,
					oAtt.Usage,
					oAtt.OrderBy,
					oAtt.InstanceSelection,
					oAtt.DefaultMember,
					AttributeUniqueNameStyle = oAtt.MemberNamesUnique,
					oAtt.KeyUniquenessGuarantee,
					oAtt.IsAggregatable,
					oAtt.AttributeHierarchyEnabled,
					oAtt.AttributeHierarchyVisible,
					oAtt.AttributeHierarchyOrdered,
					oAtt.AttributeHierarchyOptimizedState,
					oAtt.AttributeHierarchyDisplayFolder,
					oAtt.GroupingBehavior,
					KeyColumns = oAtt.KeyColumns.SelectMany(cCol => cCol.Source.Select(tCol => new{
						tCol.TableName, 
						tCol.ColumnName,
						SourceExpressions = SourceExpressions
							.Where(src => src.DataSourceViewId.Equals(oDim.Source))
							.Where(src => src.TableName.Equals(tCol.TableName))
							.Where(src => src.ColumnName.Equals(tCol.ColumnName))
							.Select(src => src.Expression).Distinct()
						
						})),
					NameColumn = oAtt.NameColumn.SelectMany(cCol => cCol.Source.Select(tCol => new{
						tCol.TableName, 
						tCol.ColumnName,
						SourceExpressions = SourceExpressions
							.Where(src => src.DataSourceViewId.Equals(oDim.Source))
					...
					HasDescendants = oAtt.AttributeRelationships.Any(),
					oAtt.AttributeRelationships,
					
					}))),
			NaturalHierarchies = cDim.Attributes
			.Where(cAtt => bool.Parse(cAtt.AttributeHierarchyVisible))
			.Join(
				oDim.Attributes,
				cAtt => cAtt.AttributeID,
				oAtt => oAtt.ID,
				(cAtt,oAtt) => new{
					CubeName = tt.Name,
					HierarchyType = "NaturalHierarchy",
					Dimension = oDim.Name,
					Hierarchy = oAtt.Name,
					Level = String.Empty,
					DataSourceViewID = oDim.Source,
			...
			).OrderBy(oNm => oNm.Hierarchy)
		}
	));
		

	CubeSet.SelectMany(cSet => cSet.AttributeHierarchies)
		.Where(cAtt => cAtt.KeyColumns.Where(kc => kc.SourceExpressions.Any()).Any())
		.Select(cAtt => new{
			cAtt.CubeName,
			cAtt.HierarchyType,
			cAtt.Dimension,
			cAtt.Hierarchy,
			cAtt.Level,
			cAtt.DataSourceViewID,
			cAtt.SourceDimensionName,
			cAtt.CubeDimensionName,
			cAtt.SourceAttribute,
			KeyColumns = cAtt.KeyColumns.Where(kc => kc.SourceExpressions.Any()),
			cAtt.NameColumn
		}).Union(CubeSet.SelectMany(cSet => cSet.NaturalHierarchies)
		.Where(cAtt => cAtt.KeyColumns.Where(kc => kc.SourceExpressions.Any()).Any())
		.Select(cAtt => new{
			cAtt.CubeName,
			cAtt.HierarchyType,
		...
		//.Where(f => f.CubeName == "COD" )
		.Where(f => 
			//f.Dimension == "Organization"
			//|| f.Dimension == "Customers"
			f.Dimension == "Maturity Date"
			)
