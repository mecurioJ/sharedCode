...
void Main()
{
	...
	//Generally, the column list is the measures used
	var ColumList = new List<String>(){"[Measures].[Current Balance]"};
	
	//Generally, the rows are the groupings and hierarchies ("slices of data")
	var RowList =  new List<String>(){"[Accounts].[Accounts].[All Accounts]","[Customers].[Customer Names].[A-Z].&[A]","[Products].[Products].[Product Family].&[10]&[Deposits]"};
	
	//filters are against dimensions that are not included in the consequenting query...
	var FilterList = new List<String>(){"[Organization].[Organization].[Charter].&[CHARTER WEST]"};
	
	
	var memberList = new List<String>{"[Accounts].[Accounts].[All Accounts]","[Customers].[Customer Names].[A-Z].&[A]","[Organization].[Organization].[Charter].&[CHARTER WEST]"};
	
	
	var cubeName = "DDA";
	var measureMeasureGroup	= "DDA Facts";
	
			
			//  
			var parseTable = "dbo_dim_Customer";
			var parseColumn = "CustomerName_A-Z";
			
			
			dataTableExpressions.SelectMany(ext => ext.ExpressionTree)
			.Where(exp => !String.IsNullOrEmpty(exp.Expression))
			.Where(tbl => tbl.TableName.Equals(parseTable))
			.Where(col => col.ColumnName.Equals(parseColumn))
			.Select(final => new{
				final.TableName,
				ColumnNames = final.ExpressionTree.Intersect( dataTableDefinitions.SelectMany(dtDef => dtDef.Tables.Where(tbl => tbl.TableName.Equals(parseTable)).Where(exp => String.IsNullOrEmpty(exp.Expression)))
			.Select(col => col.ColumnName))
			}).SelectMany(fin => fin.ColumnNames.Select(cNm => new SchemaParser.TableColumn{
				TableName = fin.TableName,
				ColumnName = cNm
			}
			))
			.Distinct()
	...
	var HierarchyByLevelDefinitions = SchemaParser.GetHierarchyByLevelDefinition(OlapData);
	var AttributeDef = SchemaParser.GetAttributeDefinitions(OlapData);
	var MeasureDef = SchemaParser.GetMeasureGroupMeasureDefinitions(OlapData);
	
	
	var DimensionIDs = new List<String>(){"Dim Accounts","Dim Customer","Dim Branch"};
	SchemaParser.GetMeasureGroupDimensionDefinitions(OlapData)
	.Where(mgDim => mgDim.CubeName.Equals("DDA") && mgDim.AttributeType.Equals("Granularity"))
	.Where(mgDim => DimensionIDs.Contains(mgDim.AttributeID))
	;
	//AttributeDef.Dump();
	...
	RowList.AddRange(FilterList);
	//memberList.AddRange(ColumList);
	

	var levelDefs = memberList.Select(rl => {
		var HierarchyLevel = Regex.Split(rl,ValueParse).First();
		var ValueDefinition = Regex.Split(rl,ValueParse).Last();
		var LevelDefinition = Regex.Split(HierarchyLevel.Substring(1,HierarchyLevel.Length-2),MeasureParse).ToList();
		LevelDefinition.Add(ValueDefinition);
		LevelDefinition.Add(HierarchyLevel);
	return LevelDefinition.ToArray();
	});
	
	levelDefs.Select(lvl => 
			{
				var Definitions = new SchemaParser.MemberDefinition {
									Dimension = lvl[0],
									Hierarchy = lvl[1],
									Level = lvl[2],
									Value = (
										String.Format("[{0}].[{1}].[{2}]",lvl[0],lvl[1],lvl[2]).Equals(lvl[3])
										|| String.Format("[{0}].[{1}]",lvl[0],lvl[1]).Equals(lvl[3])
										)
											? String.Empty
											: lvl[3],
									AttributeDefinitions = AttributeDef
																.Where(attDef => attDef.DimensionName.Equals(lvl[0])
																	&& attDef.AttributeName.Equals(lvl[1])
																	&& attDef.AllAttributeName.Equals(lvl[2])
																),
									LevelDefinitions = HierarchyByLevelDefinitions
														.Where(lvlDef => lvlDef.FullLevelName.Equals(lvl.Last())
														|| lvlDef.AttributeAllMemberName.Equals(lvl.Last())
														|| lvlDef.HierarchyAllMemberName.Equals(lvl[2])) ,
									MeasureDefinitions = MeasureDef
										.Where(md => md.CubeName.Equals(cubeName))
										.Where(md => md.MeasureGroupName.Equals(measureMeasureGroup)),
										//.Where(md => md.MeasureName.Equals(lvl[1])),
	...
	//OlapData.Dimensions.Where(dim => dim.Name.Contains("Customer")).Dump();
	
	//var MeasureDefinitions = SchemaParser.GetMeasureGroupMeasureDefinitions(OlapData).Where(cb => cb.CubeName.Equals(cubeName))
	//.Where(mg => mg.MeasureGroupName.Equals(measureMeasureGroup))
	//.Where(mn => mn.MeasureName.Equals("Current Balance"))
//	;
//	var MeasureGroupDimensionDefinitions = SchemaParser.GetMeasureGroupDimensionDefinitions(OlapData)
//	.Where(cb => cb.CubeName.Equals(cubeName))
//	.Where(mg => mg.AttributeType.Equals("Granularity"))
//	;
	
	//MeasureDefinitions.Dump();
	//MeasureGroupDimensionDefinitions.Dump();
	/*
	SchemaParser.GetDataSourceViewExpressions(OlapData)
	.SelectMany(dsv => dsv.ExpressionTree.Where(ex => !String.IsNullOrEmpty(ex.Expression))
		.Select(exp => new{exp.TableName,exp.ColumnName,exp.Expression})
	)
	.Dump();
	*/
        ...
        cDim.Hierarchies.SelectMany(cdh =>
            cdh.Levels.SelectMany(cLvl => cDim.Attributes
                ...
                .Select(cAtt => new SchemaParser.HierarchyByLevelDefinition
                {
                    ...
                    AttributeAllMemberName = String.Format("[{0}].[{1}].[{2}]", cDim.Name, cdh.Name, cDim.AttributeAllMemberName),
