...
void Main()
{
	...
	var measureDefinitions = OlapData.Cubes.SelectMany(cub => cub.MeasureGroups
	.SelectMany(mg => mg.Measures.SelectMany(mea => mea.Source.SelectMany(mSrc => mSrc.Source.Select(src => new {
				CubeName = cub.Name,
				CubeId = cub.ID,
				MeasureGroupName = mg.Name,
				MeasureGroupID = mg.ID,
				MeasureGroupIgnoreUnrelatedDimensions = mg.IgnoreUnrelatedDimensions,
				MeasureName = mea.Name,
				MeasureID = mea.ID,
				MeasureDescription = mea.Description,
				MeasureAggregateFunction = mea.AggregateFunction,
				MeasureDataType = mea.DataType,
				MeasureIsVisible = mea.Visible,
				MeasureExpression = mea.MeasureExpression,
				MeasureDisplayFolder = mea.DisplayFolder,
				MeasureFormatString = mea.FormatString,
				TableName = src.TableName,
				ColumnName = src.ColumnName,
				ColumnDataType = mSrc.DataType,
	...
	//SourceExpressions.Dump();
			
	measureDefinitions
	//.Select(mDef => mDef.AggregateFunction).Distinct()
	.Dump();
