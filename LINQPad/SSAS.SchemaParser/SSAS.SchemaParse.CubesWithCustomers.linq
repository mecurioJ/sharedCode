...
void Main()
{
	SchemaParser.oDataBase OlapData = new SchemaParser.oDataBase(XElement.Load(@"c:\Projects\SSAS\BankerSchema.xml"));
		...
		var CubeWithCustomerList = OlapData.Cubes
								//.Where(cub => cub.Dimensions.Where(cDim => cDim.AttributeType.Equals("Granularity")))
								.Where(cub => cub.Dimensions.Where(cDim => cDim.ID.Contains("Customer")).Any())
								.Where(cub => cub.Name.Equals("Loans")).Any()
								.Dump();
	...
	//the following items are attribute hierarchies and are accounted differently...
	"[Customers].[Officer Name].[Officer Name]",
	"[Accounts].[Accounts].[Accounts]",
	};
	
	var MemberSet = MemberItems
	.Select(mSet => mSet.Split('.'))
	.Select(member => new{
		Dimension = member[0].ToString().Replace("[",String.Empty).Replace("]",String.Empty),
		Hierarchy = member[1].ToString().Replace("[",String.Empty).Replace("]",String.Empty),
		Level = (member.Count() > 2 ) ? member[2].ToString().Replace("[",String.Empty).Replace("]",String.Empty) : string.Empty
	})
	.Where(dim => dim.Dimension != "Measures");
	
	var MeasureSet = MemberItems
	.Select(mSet => mSet.Split('.'))
	.Select(member => new{
		Dimension = member[0].ToString().Replace("[",String.Empty).Replace("]",String.Empty),
		Hierarchy = member[1].ToString().Replace("[",String.Empty).Replace("]",String.Empty),
	...
	.Where(dim => dim.Dimension.Equals("Measures"))
	.Select(h => h.Hierarchy)
	;
	
	
	//Get the requested tables and columns, plus any other required information...
	var CubeItem = OlapData.Cubes.Where(cub => cub.Name.Equals(cubeName))
					.SelectMany(mes => mes.MeasureGroups.SelectMany(mg => mg.Measures).Where(ms => MeasureSet.Contains(ms.Name)))
					.SelectMany(mes => mes.Source.SelectMany(kc => 
					kc.Source.Select(src => new{
							Caption = mes.Name,
							TableName = src.TableName,
							DisplayColumn = src.ColumnName,
			            PrimaryKey = SchemaParser.GetDataSourceViews(OlapData.SchemaSource)
										.SelectMany(dsv => dsv.SelectMany(sch => sch.Schema.Tables.Cast<DataTable>()
											 .Where(dt => dt.TableName.Equals(src.TableName))
											.SelectMany(dt => dt.PrimaryKey.Cast<DataColumn>()
												.Select(dc => new SchemaParser.TableColumn()
												{
													TableName = dc.Table.TableName,
...
// Define other methods and classes here
    ...
    public static class SchemaParser
    {
		...
		public class CustomerDrillThroughDefinition
		{
			...
			public IEnumerable<SchemaParser.TableColumn> PrimaryKey  {get; set;} 
			public IEnumerable<SchemaParser.TableColumn> DisplayColumns  {get; set;} 
			public IEnumerable<SchemaParser.KeyColumn> KeyColumns  {get; set;} 
			public IEnumerable<SchemaParser.KeyColumn> NameColumn  {get; set;} 
		}
