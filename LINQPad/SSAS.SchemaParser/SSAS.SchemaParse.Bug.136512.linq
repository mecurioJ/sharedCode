...
void Main()
{
	...
	//SchemaParser.GetDataSourceViewExpressions(OlapData).Dump();
	//SchemaParser.GetHierarchyByLevelDefinition(OlapData).Dump();
	
	SchemaParser.GetAttributeDefinitions(OlapData).Dump();
	
            //Get definition of measures by measure group
            var MeasureDefinitions = SchemaParser.GetMeasureGroupMeasureDefinitions(OlapData).Dump();
			...
			//OlapData.Cubes.Where(cb => cb.Name.Equals("GL Cube")).Dump();
			
			MeasureGroupDimensions.Where(cb => cb.CubeName.Equals("GL Cube")).Dump();
			
	            var selectedMeasureDefinition = MeasureDefinitions
                .Where(cb => cb.CubeName.Equals("GL Cube"))
                //.Where(mg => mg.MeasureGroupName.Equals("GL Facts"))
				.Dump();
