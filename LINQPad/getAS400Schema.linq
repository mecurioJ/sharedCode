...
void Main()
{
	...
	var tableSet = 
	as400Set.Tables["Tables"].Rows.Cast<DataRow>()
	.Select(dr => new{ TableName = dr["TABLE_NAME"], Remark = dr["Remarks"]})
	;
	
	tableSet.Dump();
	
	//.Where(t => Tables.Contains(t["TABLE_NAME"].ToString()))
	/*
	var tables = 
	as400Set.Tables["Columns"].Rows.Cast<DataRow>()
	//.Where(dr => dr.Field<String>("TABLE_NAME").ToUpper().Equals(tableName))
	.Where(dr => Tables.Contains(dr.Field<String>("TABLE_NAME").ToUpper()))
	.Select(dr => new{
		Schema = dr.Field<String>("TABLE_SCHEM").ToString(),
		TableName = dr.Field<String>("TABLE_NAME"),
		ColumnName = dr.Field<String>("COLUMN_NAME").ToString(),
		...
		//TableRemark = tableSet.Where(ts => ts.TableName.Equals(dr.Field<String>("TABLE_NAME"))).FirstOrDefault().Remark,
...
//		IsNullable = dr.Field<String>("IS_NULLABLE")
	...
	.GroupBy(g => new{g.Schema,g.TableName}, g => new{g.ExtendedProperty})
	//.Where(k => k.Key.TableName.Equals("BEDSSENRP"))
	.SelectMany(k => k.Select(d => d.ExtendedProperty))
	.Skip(9000).Take(1000)
	.Dump()
	;
	*/
...
//	
	...
	{
		Table tb = new Table(ECSDM_Staging,t.Key.Item3,t.Key.Item2);
		foreach (var column in t.Select(col => new Column(tb,col.Name, col.ColumnDataType)))
		{
...
{
	get{
		return new List<String>("ADEFPORG","ADEFP","ADEFP_S15","AHLPP","AHEPP","AWHLPP"
...
public DataType OdbConvert(String dataType, string columnSize, string scale, string precision)
{
	switch(dataType)
	{
		...
		case "1":
			return DataType.Char(int.Parse(columnSize));
			break;
