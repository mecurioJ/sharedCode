...
var MemberNames = new List<String>(){"Customer Name", "All Accounts", "Date", "Product Type",};
string measureGroup = "DDA Facts";
string measure = "Current Balance";	

	SchemaParser.oDataBase OlapData = new SchemaParser.oDataBase(XElement.Load(@"C:\Projects\SSAS\BankerSchema.xml"));
	
	//SchemaParser.GetAttributeDefinitions(OlapData).Dump();
var AttributeList = OlapData.Dimensions.SelectMany(cDim => cDim.Attributes.Select(cAtt => new
                           {
						   		DimensionId = cDim.ID,
                               cDim.Source,
                               cAtt.ID,
                               cAtt.Name,
                               KeyColumns = cAtt.KeyColumns.SelectMany(aKc => aKc.Source),
                               NameColumn = cAtt.NameColumn.SelectMany(aKc => aKc.Source),
							   Schema = OlapData.DataSourceViews.Where(ds => ds.ID.Equals(cDim.Source)).First().Schema.Tables.Cast<DataTable>()
							   				.Where(dt => dt.TableName.Equals(cAtt.KeyColumns.SelectMany(aKc => aKc.Source).First().TableName))
											.Select(tbl => new {
												tbl.TableName,
												Column = tbl.Columns
																.Cast<DataColumn>()
																.Where(dCol => dCol.ExtendedProperties["DbColumnName"].Equals(cAtt.KeyColumns.SelectMany(aKc => aKc.Source).First().ColumnName))
																.Select(dCol => new
																{
																	dCol.ColumnName,
																	Properties = dCol.ExtendedProperties
																}).ToArray()
											,
												ParentRelations = tbl.ParentRelations.Cast<DataRelation>().ToList(),
												ChildRelations = tbl.ChildRelations.Cast<DataRelation>().ToList()
											})
                           })
                       )
                       .Join(
...
AttributeList.Dump();					   
        ...
        /// <summary>
        ///     Returns a distinct list based on the values returned by the specified selector function.
        ///     Used for generating distinct lists based on a single property, even if other properties vary.
        /// </summary>
    ...
    public static class SchemaParser
    {
		...
		{
			return olapData.Dimensions.SelectMany(cDim => cDim.Attributes.Select(aInfo => new SchemaParser.AttributeDefinition{
