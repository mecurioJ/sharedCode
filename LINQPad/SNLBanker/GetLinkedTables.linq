...
void Main()
{
	...
	var DSVDef = SchemaParser.GetDataSourceViewExpressions(OlapData);
	
	var selectedMeasureDefinition = new[]{
		new{ DimensionId="[Measures]", AttributeId ="FACT DDA", TableId ="dbo_FACT_DDA", ColumnId ="Current_Balance", Type="Key" }
	}.ToList();
	
var selectedMemberDefinitions = new []{
	new{ DimensionId="Dim Accounts", AttributeId ="Dim Accounts", TableId ="dbo_dim_Accounts", ColumnId ="dimAccountID", Type="Key"},
	new{ DimensionId="Dim Customer", AttributeId ="Customer Name A-Z", TableId ="dbo_dim_Customer", ColumnId ="CustomerName", Type="Key"},
	new{ DimensionId="Dim Branch", AttributeId ="Branch Charter", TableId ="dbo_dim_Branch", ColumnId ="Bank_Name", Type="Key"},
	new{ DimensionId="Accounts", AttributeId ="Dim Accounts", TableId ="dbo_FACT_DDA", ColumnId ="dimAccountID", Type="Key"},
	new{ DimensionId="Organization", AttributeId ="Dim Branch", TableId ="dbo_FACT_DDA", ColumnId ="BranchID", Type="Key"},
	new{ DimensionId="Customers", AttributeId ="Dim Customer", TableId ="dbo_dim_AccountCustomers", ColumnId ="dimCustomerID", Type="Key"},
	new{ DimensionId="Accounts", AttributeId ="Dim Accounts", TableId ="dbo_dim_AccountCustomers", ColumnId ="dimAccountID", Type="Key"},
	new{ DimensionId="Accounts", AttributeId ="Dim Accounts", TableId ="dbo_FACT_DDA", ColumnId ="dimAccountID", Type="Key"},
	new{ DimensionId="Organization", AttributeId ="Dim Branch", TableId ="dbo_FACT_DDA", ColumnId ="BranchID", Type="Key"},
	new{ DimensionId="Accounts", AttributeId ="Dim Accounts", TableId ="dbo_Fact_ChannelUsage", ColumnId ="dimAccountID", Type="Key"},
	new{ DimensionId="Accounts", AttributeId ="Dim Accounts", TableId ="dbo_Fact_ChannelUsage", ColumnId ="dimAccountID", Type="Key"}
}.ToList();
var MeasureTableName = selectedMeasureDefinition.Select(t => t.TableId).Distinct().ToList();
var MemberTableNames = selectedMemberDefinitions.Select(t => t.TableId).Distinct().Except(MeasureTableName).ToList();

var MeasureToMember =  
SchemaParser.GetDataSourceViewDefinitions(OlapData)
.SelectMany(rel => rel.Relations.Where(tc =>
	MemberTableNames.Contains(tc.Parent.TableName)
	|| MemberTableNames.Contains(tc.Child.TableName)
))
.Where(memRel => 
	MeasureTableName.Contains(memRel.Parent.TableName)
	|| MeasureTableName.Contains(memRel.Child.TableName))
	;

var MembertoMember = 
SchemaParser.GetDataSourceViewDefinitions(OlapData)
.SelectMany(rel => rel.Relations.Distinct().Where(tc =>
	MemberTableNames.Contains(tc.Parent.TableName) && MemberTableNames.Contains(tc.Child.TableName)
));

var expressionDef = DSVDef.SelectMany(dsv => dsv.ExpressionTree.Where(dd => !String.IsNullOrEmpty(dd.Expression)));



expressionDef.Dump();

var LeftJoin =MeasureToMember.Select(mm => mm.Parent).Union(MembertoMember.Select(mm => mm.Parent)).Select(t => new{t.TableName,t.ColumnName}).Distinct();
var RightJoin = MeasureToMember.Select(mm => mm.Child).Union(MembertoMember.Select(mm => mm.Child)).Select(t => new{t.TableName,t.ColumnName}).Distinct();

LeftJoin.Select(js => new{
                    TableId = js.TableName,
                    ColumnId = js.ColumnName,
					ExpressionTree = expressionDef.Where(edef => edef.TableName.Equals(js.TableName)).Where(edef => edef.ColumnName.Equals(js.ColumnName))
}).Dump();

RightJoin.Select(js => new{
                    TableId = js.TableName,
                    ColumnId = js.ColumnName
}).Dump();
...
.SelectMany(meMem => {
		...
		return (MeasureTableName.Contains(meMem.Child.TableName))
		? MembertoMember.Where(mem => mem.Parent.TableName.Equals(meMem.Parent.TableName))
		: MembertoMember.Where(mem => mem.Child.TableName.Equals(meMem.Parent.TableName));
})
.Where(tc => MemberTableNames.Contains(tc.Parent.TableName) && MemberTableNames.Contains(tc.Child.TableName))
