...
void Main()
{
	...
	var MeasuresWithExpressions = 
	OlapData.Cubes
	//.Where(cub => cub.Name.Equals(CubeName))
	.SelectMany(cub => cub.MeasureGroups.SelectMany(mg => 
			mg.Measures.Select(mes => new{
			CubeName = cub.Name,
			MeasureGroupName = mg.Name,
			MeasureGroupID = mg.ID,
//			MeasureGroupState = mg.State,
//			MeasureGroupType = mg.Type,
//			MeasureGroupStorageLocation = mg.StorageLocation,
//			MeasureGroupIngnoreUnrelatedDimensions = mg.IgnoreUnrelatedDimensions,
//			MeasureGroupEstimatedRows = mg.EstimatedRows,
//			MeasureGroupProcessingMode = mg.ProcessingMode,
//			MeasureGroupEstimatedSize = mg.EstimatedSize,
			MeasureName = mes.Name,
			mes.ID,
			mes.Description,
			mes.AggregateFunction,
			mes.DataType,
			mes.Visible,
			mes.MeasureExpression,
			mes.DisplayFolder,
			mes.FormatString,
			mes.Translations,
			Source = mes.Source.SelectMany(mSrc => mSrc.Source
				.Select(tCol => new{
					tCol.TableName,
					tCol.ColumnName,
					SourceExpressions = 
					SourceExpressions
						.Where(sExp => sExp.TableName.Equals(tCol.TableName))
						.Where(sExp => sExp.ColumnName.Equals(tCol.ColumnName))
						.Select(sExp => sExp.Expression).FirstOrDefault()
				
				})).FirstOrDefault()
		})))
		.Where(exSrc => exSrc.Source.SourceExpressions != null ) 
	.Dump();
	//Dimensions
	//MeasureGroups
	...
	//find measures with Expressions 
	//find members with expressions in keys
}
