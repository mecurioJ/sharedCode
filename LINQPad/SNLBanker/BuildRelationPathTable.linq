...
void Main()
{
	...
	SchemaParser.oDataBase OlapData = new SchemaParser.oDataBase(XElement.Load(@"b:\Projects\SSAS\BankerSchema.xml"));
	var dataSourceViewDef = SchemaParser.GetDataSourceViewDefinitions(OlapData);
	var CurrentCube = OlapData.Cubes.Where(cub => cub.Name.Contains("DDA")).First();

var MemberListUniqueNames = new List<String>(){
	"[Customers].[Customer Names].[Customer Name].&[15823]",
	"[Accounts].[Accounts].[All Accounts]",
	"[Calendar Period].[Calendar Period].[Date].&[142]",
	"Products].[Products].[Product Type].&[CDs]",
	"[Products].[Products].[Product Type].&[Deposits]"};

var measureGroupName = "FACT DDA";
var measureUniqueName = "[Measures].[Current Balance]";
var dataSourceViewId = "DDA";

var leftJoinTable = SchemaParser.GetMeasureSourceDefinitions(OlapData).Where(mg => mg.MeasureGroupId.Equals(measureGroupName))
.Where(mm => mm.MeasureName.Equals(SchemaParser.ParseUniqueName(measureUniqueName)))
.First().TableName;
...
//leftJoinTable = "dbo_dim_Date";
	...
	SecondJoinSet.Dump();
		...
		/*
	//find all the navigation to the right join table:
	var interceptRight = RelationTable.Where(rt => rt.ParentTable.Equals(rightJoinTable) || rt.ChildTable.Equals(rightJoinTable)).Dump();

	//get all the tables in the left intercept:
	var leftTables = interceptLeft.Select(il => il.ParentTable).Distinct().Dump();
	
	//get all the tables in the right intercept:
	var rightTables = interceptRight.Select(il => il.ChildTable).Distinct().Dump();
*/
...
/*----------------------------------------------------------------------------------------------*/
	/*
	                    ...
	                    DimensionId = TablesToDimensions[rt.TableName].Select(d => d.DimensionId).FirstOrDefault(),
                    AttributeId = TablesToDimensions[rt.TableName].Select(d => d.AttributeId).FirstOrDefault(),
                    TableId = FixTableName(rt.TableName),
                    ColumnId = rt.ColumnName,
                    Type = SourceColumnType.Key,*/
	
	//SchemaParser.FindPath(dataSourceViewDef,dataSourceViewId,leftJoinTable,"dbo_dim_Accounts").Dump();
...
public class RelationPath
{
    ...
    public static class SchemaParser
    {
		...
		public static IEnumerable<SchemaParser.Relation> FindPath(IEnumerable<DataSourceViewDefinition> dataSourceViewDef, 
			String dataSourceViewId,
			String leftTableName, 
			String rightTablename)
		{
			var LeftJoinRelations = 
	dataSourceViewDef.Where(dsv => dsv.DataSourceViewId.Equals(dataSourceViewId))
		.SelectMany(dRel => dRel.Relations.Where(dr => 
			dr.Parent.TableName.Equals(leftTableName)
			|| dr.Child.TableName.Equals(leftTableName)
			));
	
	var RightJoinRelations =
		dataSourceViewDef.Where(dsv => dsv.DataSourceViewId.Equals(dataSourceViewId))
			.SelectMany(dRel => dRel.Relations.Where(dr => 
				dr.Parent.TableName.Equals(rightTablename)
				|| dr.Child.TableName.Equals(rightTablename)
				));			
				
				
				
	var IntermediateLeft =
	LeftJoinRelations
		.Select(p => p.Parent.TableName).Union(LeftJoinRelations.Select(p => p.Child.TableName))
	.SelectMany(tName => dataSourceViewDef.Where(dsv => dsv.DataSourceViewId.Equals(dataSourceViewId))
			.SelectMany(dRel => dRel.Relations.Where(dr => 
				dr.Parent.TableName.Equals(tName)
				|| dr.Child.TableName.Equals(tName)
				)))	;

				var IntermediateRight = RightJoinRelations
			.Select(p => p.Parent.TableName).Union(RightJoinRelations.Select(p => p.Child.TableName))
			.SelectMany(tName => dataSourceViewDef.Where(dsv => dsv.DataSourceViewId.Equals(dataSourceViewId))
					.SelectMany(dRel => dRel.Relations.Where(dr => 
						dr.Parent.TableName.Equals(tName)
						|| dr.Child.TableName.Equals(tName)
						))).SelectMany(ir => IntermediateLeft.Where(il => il.Parent.Equals(ir.Parent) && il.Child.Equals(ir.Child)));
...
//        }
	...
	return ActionableCubeDataSource.Dimensions.Join(
        ...
        public class Relation
        {
			...
			public bool Relates(Relation compare)
			{
				return  Parent.TableName.Equals(compare.Child.TableName) || Child.TableName.Equals(compare.Parent.TableName) ;
			}
        ...
        public class TableColumn
        {
			...
			{
				return 
					this.TableName.Equals(Compare.TableName)
					&& this.ColumnName.Equals(Compare.ColumnName);
			}
